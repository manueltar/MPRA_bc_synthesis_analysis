...
cummulative_CT_factor4_CLASS_and_Log_Rank_test_E_Plus_ASE = function(option_list)
{
  ...
  df_CSQ_colors$color[df_CSQ_colors$factor4_CLASS == "TFBS"]<-"red"
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(CUMMULATIVE_CLASSES$factor4_CLASS)))))
  cat("\n")
  cat(sprintf(as.character(summary(CUMMULATIVE_CLASSES$factor4_CLASS))))
  cat("\n")
  ...
  RESCUE_NCGR<-CUMMULATIVE_CLASSES[which(CUMMULATIVE_CLASSES$factor4_CLASS == "NCGR"),]
  ...
  if(Condition_DEBUG == 1)
  {
    ...
    cat("\n")
    cat(sprintf(as.character(names(summary(as.factor(CUMMULATIVE_CLASSES_restricted$factor4_CLASS))))))
    cat("\n")
    cat(sprintf(as.character(summary(as.factor(CUMMULATIVE_CLASSES_restricted$factor4_CLASS)))))
    cat("\n")
  }
  ...
  levels_factor4_CLASS<-c("High_AT","High_GC","Medium","Medium;High_AT","Medium;High_GC","Medium;High_GC;High_AT")
  ...
  CUMMULATIVE_CLASSES_restricted_subset<-unique(CUMMULATIVE_CLASSES_restricted[,c(which(colnames(CUMMULATIVE_CLASSES_restricted) == "VAR"),
                                                                                ...
                                                                                which(colnames(CUMMULATIVE_CLASSES_restricted) == "factor4_CLASS"),
  ...
  Log_rank_df$factor4_CLASS<-Log_rank_df$factor4_CLASS
  
  Log_rank_df$factor4_CLASS<-factor(Log_rank_df$factor4_CLASS,
                            levels=levels_factor4_CLASS,
                            ordered=T)
  ...
  CUMMULATIVE_CLASSES_restricted.dt<-data.table(CUMMULATIVE_CLASSES_restricted, key=c("Cell_Type","factor4_CLASS"))
  ...
  Freq_TOTAL.dt<-data.table(Freq_TOTAL, key=c("factor4_CLASS"))
  ...
  Log_rank_df.dt<-data.table(Log_rank_df, key=c("Cell_Type","factor4_CLASS","TILE"))
  ...
  Freq_table$factor4_CLASS<-factor(Freq_table$factor4_CLASS,
                              levels=levels_factor4_CLASS,
                              ordered=T)
  ...
  Freq_table<-Freq_table[order(Freq_table$factor4_CLASS,Freq_table$Cell_Type, Freq_table$TILE),]
  ...
  for(i in 1:length(CT_levels))
  {
    ...
    factor4_CLASS_labels<-levels(Log_rank_df_sel$factor4_CLASS)
    
    for(k in 1 :length(factor4_CLASS_labels))
    {
      BAIT<-factor4_CLASS_labels[k]
      ...
      for(h in 1 :length(factor4_CLASS_labels))
      {
        
        PREY<-factor4_CLASS_labels[h]
        ...
        Log_rank_df_sel_subset<-Log_rank_df_sel[which(Log_rank_df_sel$factor4_CLASS%in%Mini_comparison),]
        ...
        FLAG_diversity<-as.integer(length(unique(Log_rank_df_sel_subset$factor4_CLASS)))
        ...
        if(FLAG_diversity >1)
        {
          ...
          if(FLAG_diversity_2 >1)
          {
            ...
            STATS_test<-logrank.test(
              ...
              Log_rank_df_sel_subset$factor4_CLASS,
            ...
            colnames(a.df)<-c("factor4_CLASS_c1","factor4_CLASS_c2","minus_logpval")
      ...
      }# h in 1 :length(unique(Log_rank_df_sel$factor4_CLASS)
    ...
    }#k in 1 :length(unique(Log_rank_df_sel$factor4_CLASS))
    ...
    levels_CSQ_sel<-unique(Freq_table_sel$factor4_CLASS)
    ...
    df_CSQ_colors_sel<-df_CSQ_colors[which(df_CSQ_colors$factor4_CLASS%in%levels_CSQ_sel),]
    
    cat("df_CSQ_colors_sel_1\n")
    cat(str(df_CSQ_colors_sel))
    cat("\n")
    
    vector_colors_factor4_CLASS<-c("red","black","blue","dark cyan","magenta4","yellow2","gray") 
   
    
    path5<-paste(out2,'CT_factor4_CLASS_CUMULATIVE_CURVES','/', sep='')
    ...
    indx_factor4_CLASS<-which(colnames(Freq_table_sel) == "factor4_CLASS")
    indx.yaxis<-which(colnames(Freq_table_sel) == "Perc")
    mycols <- vector_colors_factor4_CLASS
    ...
    lab <- as.character(unique(Freq_table_sel[,indx_factor4_CLASS]))
    ...
    for (iteration_graph in 1:length(lab))  {
      ...
      ind <- which(Freq_table_sel[,indx_factor4_CLASS]==lab[iteration_graph])
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="E_Plus_ASE_factor4_CLASS_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="E_Plus_ASE_factor4_CLASS_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
...
cummulative_CT_factor4_CLASS_and_Log_Rank_test_enhancer = function(option_list)
{
  suppressMessages(library("nph", lib.loc="/nfs/team151/software/manuel_R_libs_4_1/"))
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="Enhancer_factor4_CLASS_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="Enhancer_factor4_CLASS_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
...
main = function() {
  ...
  cummulative_CT_factor4_CLASS_and_Log_Rank_test_E_Plus_ASE(opt)
  cummulative_CT_factor4_CLASS_and_Log_Rank_test_enhancer(opt)
