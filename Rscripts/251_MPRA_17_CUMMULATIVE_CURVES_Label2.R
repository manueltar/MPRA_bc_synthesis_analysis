...
cummulative_CT_Label2_and_Log_Rank_test_E_Plus_ASE = function(option_list)
{
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(CUMMULATIVE_CLASSES$Label_2)))))
  cat("\n")
  cat(sprintf(as.character(summary(CUMMULATIVE_CLASSES$Label_2))))
  cat("\n")
  ...
  RESCUE_NCGR<-CUMMULATIVE_CLASSES[which(CUMMULATIVE_CLASSES$Label_2 == "NCGR"),]
  ...
  CUMMULATIVE_CLASSES_restricted_subset<-unique(CUMMULATIVE_CLASSES_restricted[,c(which(colnames(CUMMULATIVE_CLASSES_restricted) == "VAR"),
                                                                                ...
                                                                                which(colnames(CUMMULATIVE_CLASSES_restricted) == "Label_2"),
  ...
  Log_rank_df$Label_2<-Log_rank_df$Label_2
  
  Log_rank_df$Label_2<-factor(Log_rank_df$Label_2,
                            levels=c("ASSAYED_VARIANT","ASE_CTRL","Enhancer_CTRL","NCGR"),
                            ordered=T)
  ...
  CUMMULATIVE_CLASSES_restricted.dt<-data.table(CUMMULATIVE_CLASSES_restricted, key=c("Cell_Type","Label_2"))
  ...
  Freq_TOTAL.dt<-data.table(Freq_TOTAL, key=c("Label_2"))
  ...
  Log_rank_df.dt<-data.table(Log_rank_df, key=c("Cell_Type","Label_2","TILE"))
  ...
  Freq_table$Label_2<-factor(Freq_table$Label_2,
                              levels=c("ASSAYED_VARIANT","ASE_CTRL","Enhancer_CTRL","NCGR"),
                              ordered=T)
  ...
  Freq_table<-Freq_table[order(Freq_table$Label_2,Freq_table$Cell_Type, Freq_table$TILE),]
  ...
  for(i in 1:length(CT_levels))
  {
    ...
    Label_2_labels<-levels(Log_rank_df_sel$Label_2)
    
    for(k in 1 :length(Label_2_labels))
    {
      BAIT<-Label_2_labels[k]
      ...
      for(h in 1 :length(Label_2_labels))
      {
        
        PREY<-Label_2_labels[h]
        ...
        Log_rank_df_sel_subset<-Log_rank_df_sel[which(Log_rank_df_sel$Label_2%in%Mini_comparison),]
        ...
        FLAG_diversity<-as.integer(length(unique(Log_rank_df_sel_subset$Label_2)))
        ...
        if(FLAG_diversity >1)
        {
          ...
          if(FLAG_diversity_2 >1)
          {
            ...
            STATS_test<-logrank.test(
              ...
              Log_rank_df_sel_subset$Label_2,
            ...
            colnames(a.df)<-c("Label_2_c1","Label_2_c2","minus_logpval")
      ...
      }# h in 1 :length(unique(Log_rank_df_sel$Label_2)
    ...
    }#k in 1 :length(unique(Log_rank_df_sel$Label_2))
    ...
    vector_colors_Label_2<-c("red","blue","dark cyan","black")
    ...
    path5<-paste(out2,'CT_Label_2_CUMULATIVE_CURVES','/', sep='')
    ...
    indx_Label_2<-which(colnames(Freq_table_sel) == "Label_2")
    indx.yaxis<-which(colnames(Freq_table_sel) == "Perc")
    mycols <- vector_colors_Label_2
    ...
    lab <- as.character(unique(Freq_table_sel[,indx_Label_2]))
    ...
    for (iteration_graph in 1:length(lab))  {
      ...
      ind <- which(Freq_table_sel[,indx_Label_2]==lab[iteration_graph])
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="E_Plus_ASE_Label2_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="E_Plus_ASE_Label_2_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
...
cummulative_CT_Label2_and_Log_Rank_test_enhancer = function(option_list)
{
  suppressMessages(library("nph", lib.loc="/nfs/team151/software/manuel_R_libs_4_1/"))
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="Enhancer_Label2_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="Enhancer_Label_2_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
...
main = function() {
  ...
  cummulative_CT_Label2_and_Log_Rank_test_E_Plus_ASE(opt)
  cummulative_CT_Label2_and_Log_Rank_test_enhancer(opt)
